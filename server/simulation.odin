...
sim_init :: proc() {
	...
	_ = sm.dynamic_slot_map_insert_set(
		&units,
		cm.Unit{0, 0, la.identity(cm.Transform), [2]int{0, 6}},
	)
}
...
sim_gather_keyframe_for_all :: proc() -> []cm.KeyframeData {
	...
	for i in 0 ..< units.size {
		...
		res[i] = cm.KeyframeData{id, u}
	}
	return res
}

sim_set_units_target :: proc(uids : []UnitID, target : cm.Vec2i) {
	for uid in uids {
