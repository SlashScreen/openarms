...
cy_binding :: proc "c" (t : ^cy.Thread) -> cy.Ret {
	...
	res := cy.get_ret(t, Key)
	b := cy.get(t, KeyActions)
...
cy_axis2 :: proc "c" (t : ^cy.Thread) -> cy.Ret {
	context = runtime.default_context()
	res := cy.get_ret(t, [2]f32)
	x := cy.get(t, InputRange)
	y := cy.get(t, InputRange)
	res^ = key_2_axis(x^, y^)
	return .Ok
}
...
load_input_api :: proc(vm : ^cy.VM, mod : ^cy.Sym, res : ^cy.LoaderResult) -> bool {
	...
	res.src = cy.alias_string_to_bytes(#load("api/input.cy", string))
