...
cy_query_ray :: proc "c" (t : ^cy.Thread) -> cy.Ret {
	...
	res := cy.thread_get_ret(t, cy.Option(UnitID))
	ray := cy.thread_get(t, Ray)
	max_dist := cy.thread_get_prim(t, f32)
...
cy_query_ray_terrain :: proc "c" (t : ^cy.Thread) -> cy.Ret {
	context = runtime.default_context()
	res := cy.thread_get_ret(t, cy.Option(HitInfo))
	ray := cy.thread_get(t, Ray)
...
load_physics_api :: proc(vm : ^cy.VM, mod : ^cy.Sym, res : ^cy.LoaderResult) -> bool {
	...
	res.src = cy.const_bytes(#load("api/physics.cy", string))
